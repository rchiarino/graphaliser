import { ASTNode, NodeType, createValueNode, createStructureNode } from "./astParser";

function addNodeToAST(ast: ASTNode, newNodeId: string, parentNodeId: string, isParallel: boolean): ASTNode {
  function addNodeRecursive(node: ASTNode): ASTNode {
    ...
    if (node.type === NodeType.SEQ) {
      const parentIndex = node.children.findIndex(child => 
        (child.type === NodeType.VALUE && child.value === parentNodeId) ||
        (child.type !== NodeType.VALUE && child.children.some(grandchild => grandchild.type === NodeType.VALUE && grandchild.value === parentNodeId))
      );

      if (parentIndex !== -1) {
        const updatedChildren = [...node.children];
        if (isParallel) {
          if (parentIndex + 1 < updatedChildren.length && updatedChildren[parentIndex + 1].type === NodeType.PAR) {
            updatedChildren[parentIndex + 1] = {
              ...updatedChildren[parentIndex + 1],
              children: [...updatedChildren[parentIndex + 1].children, createValueNode(newNodeId)]
            };
          } else {
          ...
          updatedChildren.splice(parentIndex + 1, 0, createValueNode(newNodeId));
        }
